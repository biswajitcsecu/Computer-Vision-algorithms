...
static void addEdge(  TNodeID from, TNodeID to, const std::map<TNodeID, CPose2D>& real_poses,      CNetworkOfPoses2D& graph_links)
{
...
void TestDijkstra() {
    ...
    getRandomGenerator().randomize(40);

    // Create a random graph:    
    const size_t N_VERTEX =50;
    const double DIST_THRES =20;
    const double NODES_XY_MAX = 25;
    vector<double> xs, ys;
    ...
    auto lmb = [&]([[maybe_unused]] const TNodeID parent,
                   ...
                   const size_t depth_level) {   cout << string(depth_level * 3, ' ');
    ...
    CDisplayWindowPlots win("Dijkstra Simulation");
    win.hold_on();
    win.axis_equal();
    ...
    for (TNodeID i = 0; i < N_VERTEX && win.isOpen(); i++)    {
        ...
        win.setWindowTitle(format( "Dijkstra path %u->%u", static_cast<unsigned int>(SOURCE_NODE),
