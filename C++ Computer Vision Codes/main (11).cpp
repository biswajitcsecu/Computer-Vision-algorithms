...
// Default number of ICP iterations
   ...
   time.tic ();
   string file ="/home/picox/Projects/Netbeans/InteractiveRegIcp/src.ply";
   
   if (pcl::io::loadPLYFile (file, *cloud_in) < 0)
   {
  ...
  viewer.addText (" Original point cloud\nGreen: Matrix transformed", 10, 15, 16, txt_gray_lvl, txt_gray_lvl, txt_gray_lvl, "icp_info_1", v1);
...
{
    try {
        InteractiveReg *exobj=new InteractiveReg();
        exobj->run();
        delete exobj;
        return EXIT_SUCCESS;
    } catch (exception exp) {
